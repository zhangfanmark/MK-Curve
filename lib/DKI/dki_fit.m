function [b0, dt, fit_warning_mask] = dki_fit(dwi, grad, mask, constraints, outliers)
...
%     dwi = double(dwi);
    ...
    grad = double(grad);
    % Gradient directions: sum of the squares
    % is equal to one. If not, will be set to [0, 0, 0];
    grad(:,1:3) = bsxfun(@rdivide,grad(:,1:3),sqrt(sum(grad(:,1:3).^2,2))); 
    grad(isnan(grad)) = 0;
    bval = grad(:, 4);
    ...
    % After vectorization, dwi = number_of_directions x number of voxels in the mask.
    ...
    bS = ones(ndwis, 1);
    % grad(:,D_ind(:, 1)).*grad(:,D_ind(:, 2)) = 
    % \sum_{i=1}^{3}\sum_{i=j}^{3} n_i * n_j
    bD = D_cnt(ones(ndwis, 1), :).*grad(:,D_ind(:, 1)).*grad(:,D_ind(:, 2));
    bW = W_cnt(ones(ndwis, 1), :).*grad(:,W_ind(:, 1)).*grad(:,W_ind(:, 2)).*grad(:,W_ind(:, 3)).*grad(:,W_ind(:, 4));
    
    % This is the Matrix A, corresponding to Eq. 10 in the paper
    % Tabesh2011_esitmation_MRM
    b = [bS, -bval(:, ones(1, 6)).*bD, (bval(:, ones(1, 15)).^2/6).*bW];

    % unconstrained LLS fit
    % dt is the X, which we are going to fit.
    % y is the signal
    dt = b\log(dwi); % y = A*X -> log(dwi) = b*dt
    w = exp(b*dt);

    nvoxels = size(dwi,2);

    warning_mask = zeros(1, nvoxels);
    ...
    if any(constraints) 
        ...
        parfor i = 1:nvoxels
            ...
            if isempty(b_) || cond(b(in_, :))>1e15
                ...
                wi = w(:,i); 
                Wi = diag(wi(in_)); % estimated data
                logdwii = log(dwi(in_,i)); % real data
                lastwarn('')
    ...
    fit_warning_mask = vectorize(warning_mask, mask);
end
